...
withTCPT f (TCPT rdr) = TCPT $ withReaderT (\(pq,conn,r) -> (f <$> pq,conn,r)) rdr
...
runTCP (TCPT rdr) gtr rsrc conn = do
...
runTCP' rdr gtr conn = runTCP rdr gtr () conn
...
runTCPT (TCPT rdr) gtr rsrc conn = do
...
getConnection = TCPT $ do { (_,conn,_) <- ask ; return conn }
-- asdfzxcv

-- | Use this to access the shared resource accessible to
-- multiple connections.
getResource  :: (MonadIO m) => TCPT i r m r
getResource = TCPT $ do { (_,_,rsrc) <- ask ; return rsrc }
...
atomicallyTCPT action = TCPT $ liftIO (atomically action)
